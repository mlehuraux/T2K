/*******************************************************************************
 ...
 History:
   ...
   October 2018: added is_arc and corrected format of the list of pedestals
   or thresholds saved to file.
...
void FemArray_Clear(FemArray *fa)
{
	...
	fa->list_fr_cnt      = 0;
	fa->is_first_fr      = 0;
	fa->req_threshold    = CREDIT_THRESHOLD_FRAMES_FOR_REQ;
	fa->cred_unit        = 'F'; // Default credit unit is frames
	fa->drop_a_credit    = 0;
	fa->delay_a_credit   = 0;
...
int FemArray_Open(FemArray *fa)
{
	...
	for (i=0; i<MAX_NUMBER_OF_FEMINOS; i++)
	{
		...
		{
			if ((err = FemProxy_Open(&fa->fp[i], &(fa->loc_ip[0]), &(fa->rem_ip_beg[0]), i, fa->rem_port, fa->is_arc)) < 0)
			{
...
#endif
	{
		...
		else
		{
			...
			if (fa->daq_last_time.tv_sec != 0)
			{
				...
				{
					diff = ((now.tv_sec - fa->daq_last_time.tv_sec) * 1000000) + (now.tv_usec - fa->daq_last_time.tv_usec);
				}
				else
				{
					diff = ((now.tv_sec - fa->daq_last_time.tv_sec - 1) * 1000000) + (now.tv_usec + 1000000 - fa->daq_last_time.tv_usec);
			...
			if (diff != 0)
			{
				...
				// Result is only meaningful if positive
				if (daq_speed < 0)
				{
					daq_speed = 0.0;
				}
...
int FemArray_SavePedThrList(FemArray *fa, void *buf)
{
	...
	if (fa->pedthr)
	{
		...
		if (fa->is_arc)
		{
			vf = FRAME_PRINT_LISTS | FRAME_PRINT_LISTS_FOR_ARC;
		}
		else
		{
			vf = FRAME_PRINT_LISTS;
		}

		Frame_Print((void *) fa->pedthr, (void *) buf_s, (int) sz, vf);
	}
				...
				if (fa->fem_proxy_set & mask)
				{
					if (FD_ISSET(fa->fp[i].client, &readfds_work))
					{
						...
						if (fa->fp[i].buf_in == (unsigned char *) 0)
						{
							...
							{
								printf("FemArray_ReceiveLoop: BufPool_GiveBuffer failed (%d)\n", err);
								return(err);
							}
						}
						...
						if (fa->is_list_fr_pnd)
						{
							...
							{
								printf("FemArray_ReceiveLoop: FemArray_SavePedThrList failed (%d)\n", err);
								return(err);
							}
						}
