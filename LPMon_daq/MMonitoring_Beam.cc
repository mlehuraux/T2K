...
struct DataPacketHeader {
  unsigned short size;
  unsigned short dcchdr;			// To change if the data after May 2010
  unsigned short hdr;
  unsigned short args;
  unsigned short ts_h;
  unsigned short ts_l;
...
void CreateHisto() {
  ...
  hx144 = new TH1F("hx144", "beam at x axis", 74, -120., 120.);   //wwx
  ht144 = new TH1F("ht144", "beam at time", 512, -0.5, 511.5);   //wwx
  hxt144 = new TH2F("hxt144", "beam", 512, -0.5, 511.5, 74, -120., 120.);   //wwx
  ...
  for (int iPad=0; iPad < kPhysChannelMax; iPad++)  {
	...
	sprintf(hname, "hADC %d", hiPad);
      ...
      if ( hxtime ) {
        ...
        slope = (lastN - firstN) / 502;
        
        float maxA = 0., lasttry = 0.;
        float absdatanew = 0.;
        float newdata=0.; 
