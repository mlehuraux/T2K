...
vector<T2KPadPanel*> tpcPad(kPhysChannelMax);
...
void CreateCanvases()
{
  ...
  for (int ipad=0; ipad<kPhysChannelMax; ipad++) {
    cPadPanel->Connect("ProcessedEvent(Int_t,Int_t,Int_t,TObject*)",
                       "T2KPadPanel", tpcPad[ipad],
                       "ExecuteEvent(Int_t,Int_t,Int_t,TObject*)");
    //tpcPad[ipad]->Connect("T2KPadPanelSelected(Int_t)", 0,
...
TCanvas* CreatePadPanel()
{
  ...
  const double yoffset = -0.45;     // for KeyStone
  ...
  for (int ipad=0; ipad<kPhysChannelMax; ipad++) {
    ...
    tpcPad[ipad] = new T2KPadPanel(ipad, xCorner, yCorner);
...
int ReadGeomMap()
{
  char filename[] = "T2KPadPanelGeom.txt";
