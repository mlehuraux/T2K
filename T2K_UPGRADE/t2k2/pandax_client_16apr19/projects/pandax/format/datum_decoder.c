...
int Datum_Decode(DatumContext *dc, unsigned short datum)
{
	...
	else
	{
		...
		{
			sprintf(&(dc->ErrorString[0]), "Datum(%d) PFX_EXTD_CARD_CHIP_CHAN_H_MD : interpretation not implemented.", dc->DatumCount);
			return(-1);
		}
...
Item_Print
*******************************************************************************/
int Item_Print(void *fp, DatumContext *dc, unsigned int flags)
{
	char source_type[4];
	...
	if (dc->isItemComplete)
	{
		...
		case IT_CHANNEL_HIT_HEADER:
			if (flags & IT_CHANNEL_HIT_HEADER)
			{
		...
		case IT_START_OF_BUILT_EVENT:
			if (flags & IT_START_OF_BUILT_EVENT)
			{
		...
		case IT_END_OF_BUILT_EVENT:
			if (flags & IT_END_OF_BUILT_EVENT)
			{
