...
do
  ...
  cp $path ~/suvorov/data_temp/$f_acq
  ...
  do
    ...
    echo "**********************************************"
    ~/INSTALLFCC/DataWriter_devel/bin/acqToRootConverterExe -t $j -m 1 -f  $f_acq --noxml
    mv $f_root ../data_root/$f_root_new
  done

  rm ~/suvorov/data_temp/$f_acq
done
