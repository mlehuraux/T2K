  ...
  multigen_channel=strip;
	 ...
	 if (Strip_Charge[istrip]>thr){  // found a seed now find cluster end
	   ...
	   if (cluster_size>=clusize_min) { // find 3 adjacent neighbours  istrip ,  istrip+1 ,... istrip-1+ clustersize
	     ...
	     // Now store the results  if it is the longest cluster
	     if (LengthCluster_forAsic[ThisAsic] < cluster_size){
	       LengthCluster_forAsic[ThisAsic]= cluster_size; // Keep the longest cluster
	       PosCluster_forAsic[ThisAsic]=0;
	       for (int i=istrip;  i <istrip+cluster_size ; i++) { 
		 //PosCluster_forAsic[ThisAsic]=i; old way. just keeping the last cluster.//store the position of the longest cluster
		 if (charge_tot>0)
		 PosCluster_forAsic[ThisAsic]+=i*Strip_Charge[i]/charge_tot; 		 		 
	       }
	     }		 
		    N_cluster++;
